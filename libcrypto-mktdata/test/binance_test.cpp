...
using cloudwall::binance::marketdata::BinanceRawFeedClient;
...
TEST(BinanceRawFeedClient, connect) {
    auto ccy_pair = CurrencyPair(Currency("BTC"), Currency("USDT"));
    ...
    auto client = BinanceRawFeedClient(sub, callback);
    client.connect();
    for (int i = 0; i < 5; i++) {
