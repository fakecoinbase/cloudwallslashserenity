...
def phemex_upload_main(behemoth_path: str = '/behemoth', days_back: int = 1):
    init_logging()
    ...
    for instrument in instr_cache.get_all_exchange_instruments('Phemex'):
        symbol = instrument.get_exchange_instrument_code()
        db = 'PHEMEX_TRADES'
        journal = Journal(Path(f'{behemoth_path}/journals/{db}/{symbol}'))
        ...
        logger.info(f'uploading journaled {symbol} ticks to Behemoth for UTC date {str(upload_date)}')
